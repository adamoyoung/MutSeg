...
def one_seg(mc_data):
	...
	T = array.shape[1]
	seg_scores = np.zeros([len(chrm_lens)], dtype=float_t)

	for i in range(len(chrm_lens) - 1):
	...
	total_C = np.sum(tumour_C)
	total_M = total_Ms[-1]
	print("chrm {}: total_C = {}, total_M = {}".format(len(chrm_lens), total_C, total_M) )
	# assert(total_C == totals_M)
	term_one =  np.sum( (tumour_C/total_M) * np.log( (tumour_C/total_M) + eps ))
	term_two = -( total_C / total_M ) * np.log( ( total_C / total_M) + eps )
	seg_scores[-1] = term_one + term_two

	for i in range(len(chrm_lens)):
...
def naive(mc_data):
	...
	# make sure that group_by == 1
	for i in range(len(mut_pos)):
		assert(len(mut_pos.shape) == 1)
	...
	T = array.shape[1]
	seg_scores = [np.zeros([ chrm_lens[i] // SEG_SIZE ], dtype=float_t) for i in range(len(chrm_lens))]
	total_num_segs = sum([seg_scores[i].shape[0] for i in range(len(seg_scores))])
	print( [chrm_lens[i] // SEG_SIZE for i in range(len(chrm_lens)) ] )
	print( total_num_segs )

	for i in range(len(chrm_begs)):
		print( "chrm {}: largest mut_pos = {}, largest segment size = {}".format(i+1, mut_pos[i][-1], (chrm_lens[i] // SEG_SIZE)*SEG_SIZE) )
		# if mut_pos[i][-1] > (chrm_lens[i] // SEG_SIZE)*SEG_SIZE:
		# 	print("warning!")
		# print("starting chromosome {}".format(i+1))
		cur_pos = 0
		...
		for j in range( 1, (chrm_lens[i] // SEG_SIZE) - 1 ):
			...
			while chrm_mut_pos[cur_pos] < end_pt:
		...
		# last iteration gets all of mutations on the chrm that haven't been added yet
