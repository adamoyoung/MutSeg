...
def compute_counts(muts, M, T):
	# assumes that muts is a [M] bitmask array
...
def total_score(muts, partition, min_size):
	...
	for i in range(1,len(partition)):
		...
		term_two = -( (total_counts/M) * np.log(total_counts/M) )
...
def brute_force(muts, pos, T, K, min_size=1):
	...
	for partition in partitions:
		...
		score = total_score(muts,partition,min_size)
